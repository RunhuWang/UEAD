...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        # snow
        # snowflakes = iaa.Snowflakes(flake_size=(0.1, 0.2), speed=(0.02, 0.05), density=(0.02, 0.1))
        # stitched_image = snowflakes(image=stitched_image)

        #gaussian noise
        # gaussian_noise = iaa.AdditiveGaussianNoise(loc=0, scale=(10, 30), per_channel=True)
        # stitched_image = gaussian_noise(image=stitched_image)

        # motionblur = iaa.MotionBlur(k=15)
        # stitched_image = motionblur(image=stitched_image)


        # Rain = iaa.Rain(drop_size=(0.10, 0.15), speed=(0.1, 0.2))
        # stitched_image = Rain(image=stitched_image)

        # PoissonNoise = iaa.AdditivePoissonNoise(lam=15, per_channel=True)
        # stitched_image = PoissonNoise(image=stitched_image)

        # Fog = iaa.Fog(seed=3)
        # stitched_image = Fog(image=stitched_image)

        # stitched_image = self._add_random_glare_effect(stitched_image)

        # saltNoise = iaa.SaltAndPepper(0.5, per_channel=True)
        # stitched_image = saltNoise(image=stitched_image)

        # Defocus = iaa.imgcorruptlike.DefocusBlur(severity=3)
        # stitched_image = Defocus(image=stitched_image)

        # GAUSSblur = iaa.GaussianBlur(sigma=3)
        # stitched_image = GAUSSblur(image=stitched_image)

        # ElasticTrans = iaa.ElasticTransformation(alpha=50, sigma=5)
        # stitched_image = ElasticTrans(image=stitched_image)

        # stitched_image = self._add_color_quantize(stitched_image, levels=8)

        # stitched_image = self._add_JPEG_quantize(stitched_image, quality=10)

        #brightness_aug = iaa.MultiplyBrightness(mul=0.9)
        # brightness_aug = iaa.AddToBrightness(add=-50)
        # stitched_image = brightness_aug(image=stitched_image)
