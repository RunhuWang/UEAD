...
from navsim.agents.WoTE.scene_graph_trajectory_reconstructor_wote import SceneGraphTrajectoryReconstructor, trajectory_reconstruction_anomaly_score
...
class WoTEModel(nn.Module):
    def __init__(self, 
        ...
        self.traj_reconstructor = SceneGraphTrajectoryReconstructor()
        #self._load_pretrained_and_setup('/home/fqz02/Works/WangRunhu/demo02/navsim_workspace/DiffusionDrive/last.ckpt')


    def _load_pretrained_and_setup(self, pretrained_path):
        """内部方法：加载权重，初始化专用专家，冻结参数"""
        # 加载原始模型的state_dict

        raw_pretrained_dict = torch.load(pretrained_path)["state_dict"]

        prefix_to_remove = "agent.WoTE_model."
        pretrained_dict = {}
        removed_count = 0
        for key, value in raw_pretrained_dict.items():
        ...
        print(f"Removed prefix '{prefix_to_remove}' from {removed_count} keys.")
        # backbone_mode = {k: v for k, v in pretrained_dict.items() if k.startswith('_backbone.')}
        # 尝试自动加载匹配的层（A、B、C 中的注意力、层归一化等）
        missing_keys, unexpected_keys = self.load_state_dict(pretrained_dict, strict=False)

        print("=" * 50)
        print("Missing keys Parameters loaded automatically via load_state_dict (from pretrained):")
        for key in sorted(set(missing_keys)):
            # 过滤掉稍后手动处理的 MoE 内部参数，避免混淆（但这些键本来也不在 loaded_keys 中）
            print(f"  {key}")
        # # # 第四步：冻结所有参数
        ...
        #
        # for param in self.agent_query_embedding.parameters():
        #     param.requires_grad = True
        #
        for param in self.traj_reconstructor.parameters():
            param.requires_grad = True

        print("Pretrained weights loaded and training setup completed.")
    ...
    def extract_trajectory_feature(self, features: Dict[str, torch.Tensor], targets=None) -> Dict[str, Any]:
        ...
        reconstructor = self.traj_reconstructor(agents["agent_states"], agents["agent_labels"], agents_query,
                                                bev_semantic_map, upsampled_bev_feature, status_feature, ego_status_feat.squeeze(1))
        results.update(reconstructor)
    ...
    def forward_test(self, features, targets=None) -> Dict[str, torch.Tensor]:
        ...
        anomaly_score = trajectory_reconstruction_anomaly_score(encoder_results["pred_all_trajectory"],
                                                                encoder_results["pred_trajectory_im_scores"],
                                                                encoder_results["pred_trajectory_sim_scores"],
                                                                encoder_results["trajectory_offset"],
                                                                im_rewards_softmax,
                                                                sim_rewards)
    ...
    def forward_train(self, features, targets=None) -> Dict[str, torch.Tensor]:
        ...
        final_rewards = self.weighted_reward_calculation(im_rewards_softmax, [reward.sigmoid() for reward in sim_rewards])
