...
from navsim.agents.WoTE.scene_graph_trajectory_reconstructor_wote import trajectory_reconstruction_loss
...
def compute_wote_loss(
    ...
    if use_agent_loss:
        ...
        agent_class_weight = config.agent_class_weight if hasattr(config, "agent_class_weight") else 10.0
        agent_box_weight = config.agent_box_weight if hasattr(config, "agent_box_weight") else 1.0
    ...
    reconstruction_loss = trajectory_reconstruction_loss(predictions["pred_all_trajectory"],
                                                         predictions["pred_trajectory_im_scores"],
                                                         predictions["pred_trajectory_sim_scores"],
                                                         predictions["trajectory_offset"],
                                                         predictions["im_rewards"],
                                                         predictions["sim_rewards"])
    loss_dict['recons_loss'] = 10.0 * reconstruction_loss["loss"]
