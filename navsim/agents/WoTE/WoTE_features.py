...
import timm, cv2
from navsim.agents.diffusiondrive_main.modules.lidar_bev_corruptions import apply_lidar_bev_corruption, get_image_corruption
...
class WoTEFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        #aug = get_image_corruption("gaussian_noise", severity=3)
        #stitched_image = aug(image=stitched_image)
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        #final_features = apply_lidar_bev_corruption(features, corruption="random_cell_dropout", severity=3).astype(np.float32)
