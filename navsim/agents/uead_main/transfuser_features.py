...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        edge_features = self._get_edge_feature(agent_input)
        features["edge_feature0"] = edge_features["Canny-Otsu"][0]
        features["edge_feature1"] = edge_features["Canny-Otsu"][1]
        features["edge_feature2"] = edge_features["Canny-Otsu"][2]

        # Expose the three density levels of the other edge operators.
        for level, tensor_edge in enumerate(edge_features["Sobel-Otsu"]):
            features[f"sobel_edge_feature{level}"] = tensor_edge
        for level, tensor_edge in enumerate(edge_features["Laplacian-Otsu"]):
            features[f"laplacian_edge_feature{level}"] = tensor_edge
    ...
    def _get_edge_feature(self, agent_input: AgentInput) -> Dict[str, List[torch.Tensor]]:
        """
        Extract stitched camera from AgentInput
        :param agent_input: input dataclass
        :return: stitched front view image as torch tensor
        ...
        return {"Canny-Otsu": self._canny_with_fixed_thresholds(resized_image,[(34,74), (62,99), (87,124)]),"Sobel-Otsu": self._sobel_with_otsu_biases(resized_image),"Laplacian-Otsu": self._laplacian_with_otsu_biases(resized_image)}

        return fixed_tensor_edges_list
    def _canny_with_otsu_biases(self, image, threshold_biases=(0, 25, 50)):
        """
        使用Otsu阈值和固定偏置生成三种边缘密度的Canny边缘图。

        Otsu阈值作为高阈值，其一半作为低阈值；随后对两个阈值施加
        相同偏置，以保持三档阈值与原固定阈值实验一致。
        """
        ...
        gradient_8u = cv2.convertScaleAbs(gradient_magnitude)

        otsu_threshold, _ = cv2.threshold(
            gradient_8u, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )
        base_high_threshold = int(otsu_threshold)
        base_low_threshold = int(base_high_threshold * 0.5)

        tensor_edges_images = []
        for bias in threshold_biases:
            low_threshold = min(255, max(0, base_low_threshold + bias))
            high_threshold = min(255, max(0, base_high_threshold + bias))
            edges = cv2.Canny(gray, low_threshold, high_threshold)
            tensor_edges_images.append(transforms.ToTensor()(edges))

        return tensor_edges_images

    def _sobel_with_otsu_biases(self, image, threshold_biases=(0, 25, 50)):
        """使用Sobel响应的Otsu阈值和固定偏置生成三档二值边缘图。"""
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            gray = image
        ...
        tensor_edges_images = []
        for bias in threshold_biases:
            threshold = min(255, max(0, int(otsu_threshold) + bias))
            _, edges = cv2.threshold(gradient_8u, threshold, 255, cv2.THRESH_BINARY)
            tensor_edges_images.append(transforms.ToTensor()(edges))

        return tensor_edges_images

    def _laplacian_with_otsu_biases(self, image, threshold_biases=(0, 25, 50)):
        """使用Laplacian响应的Otsu阈值和固定偏置生成三档二值边缘图。"""
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            gray = image
        ...
        laplacian_8u = cv2.convertScaleAbs(laplacian_magnitude)

        otsu_threshold, _ = cv2.threshold(
            laplacian_8u, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )

        tensor_edges_images = []
        for bias in threshold_biases:
            threshold = min(255, max(0, int(otsu_threshold) + bias))
            _, edges = cv2.threshold(laplacian_8u, threshold, 255, cv2.THRESH_BINARY)
            tensor_edges_images.append(transforms.ToTensor()(edges))

        return tensor_edges_images

    def _canny_with_fixed_thresholds(self, image, threshold_pairs):
        """
        使用多组固定阈值进行Canny边缘检测

        参数:
            image: 输入图像
            threshold_pairs: 阈值对列表 [(low1, high1), (low2, high2), ...]
        ...
        for low_threshold, high_threshold in threshold_pairs:
            # 应用Canny边缘检测
            edges = cv2.Canny(gray, low_threshold, high_threshold)
            tensor_edges = transforms.ToTensor()(edges)
            tensor_edges_images.append(tensor_edges)

        return tensor_edges_images

    def _canny_with_otsu_thresholds(self, image):
        """
        使用多组固定阈值进行Canny边缘检测
        ...
        # 转换为8位图像
        gradient_8u = np.uint8(gradient_magnitude)

        # 应用Otsu方法获取阈值
        otsu_threshold, _ = cv2.threshold(gradient_8u, 0, 255,
                                          cv2.THRESH_BINARY + cv2.THRESH_OTSU)

        # 设置Canny阈值
        high_threshold = min(255, otsu_threshold)
        low_threshold = max(0, high_threshold * 0.5)
