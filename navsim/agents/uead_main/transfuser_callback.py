...
class TransfuserCallback(pl.Callback):
    ...
    def _visualize_model(
        ...
        if self._config.edge:
            pred_edge = torch.sigmoid(predictions["edge2"]).permute(0, 2, 3, 1).numpy()
            ...
            for sample_idx in range(self._num_rows * self._num_columns):
                ...
                plot[128:, :512] = (np.repeat(pred_edge[sample_idx] * 255, 3, axis=2)).astype(np.uint8)[::2, ::2]
            ...
            for sample_idx in range(self._num_rows * self._num_columns):
                plot = np.zeros((256, 768, 3), dtype=np.uint8)
                plot[:128, :512] = (camera[sample_idx] * 255).astype(np.uint8)[::2, ::2]
                plot[128:, :512] = (pred_edge[sample_idx] * 255).astype(np.uint8)[::2, ::2]
